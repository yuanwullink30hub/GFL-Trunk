// Re-run the dev test account's analysis (Ruler × Outlaw) with the UPDATED master
...
//
// Usage: node scripts/rerun-dev-assessment.js [apiBase]
...
const LEVENSLES = 'Ik verscheur wat niet deugd en bouw iets eerlijkers — tot mijn betere versie haar eerste leugen nodig had en ik moest kiezen tussen hervormer blijven of heerser worden. Een verbetering die niet herzien mag worden is het oude systeem met mijn naam erop.';

// Dual-core N/C per group — from the dev account's PDF machine block (PROFIEL DATA).
const SUBGROUPS = [
...
(async () => {
  ...
  const body = {
    ...
    extendedArchetypeName: 'The Reformer',         // #6 Ruler × Outlaw (roster v1.3)
  ...
  fs.writeFileSync(require('path').join(__dirname, '..', '..', '..', 'tmp', 'dev-rerun-analysis.md'), result.analysis);
  ...
  console.log('assessment.analysis updated. Full analysis saved to tmp/dev-rerun-analysis.md');
