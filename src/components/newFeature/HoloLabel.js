...
const HoloLabel = ({ 
  ...
  return (
    <div className={`select-none flex items-center ${isRight ? 'flex-row' : 'flex-row-reverse'} group`}>
      ...
      {/* Main Card */}
      <div className={`relative bg-slate-900/90 backdrop-blur-md border border-cyan-500/50 rounded-sm shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-all duration-300 ${isRight ? 'text-left' : 'text-right'}`} style={{padding: 'clamp(1rem, 2vw, 1.5rem)', width: 'clamp(15rem, 40vw, 25rem)'}}>
        ...
        <div className={`flex items-center border-b border-cyan-500/30 ${isRight ? 'justify-between' : 'flex-row-reverse justify-between'}`} style={{marginBottom: 'clamp(0.5rem, 1vw, 1rem)', paddingBottom: 'clamp(0.25rem, 0.5vw, 0.5rem)'}}>
          <span className="text-cyan-400 font-bold tracking-widest uppercase" style={{fontSize: 'clamp(1rem, 2vw, 1.5rem)'}}>
            {`LAYER_0${layerIndex + 1} // INPUT`}
          </span>
          <span className="text-orange-400 animate-pulse" style={{fontSize: 'clamp(1.2rem, 2.5vw, 2rem)'}}>
        ...
        <div style={{display: 'flex', flexDirection: 'column', gap: 'clamp(0.75rem, 1.5vw, 1.5rem)'}}>
           ...
           <div className={`flex flex-col ${isRight ? 'items-start' : 'items-end'}`} style={{gap: 'clamp(0.25rem, 0.5vw, 0.5rem)'}}>
               <label className="text-cyan-200/70 font-mono tracking-wider" style={{fontSize: 'clamp(1.2rem, 2.5vw, 1.8rem)'}}>
               ...
               <input 
                 ...
                 style={{fontSize: 'clamp(1rem, 2vw, 1.5rem)', padding: 'clamp(0.5rem, 1vw, 1rem) clamp(0.75rem, 1.5vw, 1.2rem)'}}
               />
           </div>
           ...
           {showButton && !isSaved && !isSent && (
               <button 
                ...
                style={{marginTop: 'clamp(0.5rem, 1vw, 1rem)', padding: 'clamp(0.75rem, 1.5vw, 1.2rem) clamp(1rem, 2vw, 1.6rem)', gap: 'clamp(0.5rem, 1vw, 1rem)', fontSize: 'clamp(1.2rem, 2.5vw, 1.8rem)'}}
               >
                 <span>{isLast ? 'TRANSMIT' : 'SAVE DATA'}</span>
                 <span style={{fontSize: 'clamp(1.5rem, 3vw, 2.2rem)', lineHeight: '1'}}>›</span>
               </button>
           )}
           
           {(isSaved || isSent) && (
            <div className={`w-full font-mono border text-center ${isSent ? 'border-yellow-500/30 bg-yellow-900/20 text-yellow-400' : 'border-green-500/30 bg-green-900/20 text-green-400'}`} style={{marginTop: 'clamp(0.5rem, 1vw, 1rem)', padding: 'clamp(0.5rem, 1vw, 0.8rem)', fontSize: 'clamp(1.2rem, 2.5vw, 1.8rem)'}}>
